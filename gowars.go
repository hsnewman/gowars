...
`
	...
	http.HandleFunc("/telnet/ws", func(w http.ResponseWriter, r *http.Request) {
...
<style>
  * { box-sizing: border-box; }
  ...
  /* Top: numeric pad — 2 rows (digits row + Enter row) */
  #numpad {
    display: grid;
  ...
  /* Bottom: context-aware command buttons, scrollable */
  #action-buttons {
    display: flex;
    flex-direction: column;
    gap: 2px;
  ...
  /* Shared button styles — act-btn matches num-btn size */
  .act-btn, .num-btn {
    background: #111;
    color: #0f0;
    border: 1px solid #0f0;
  ...
  /* Enter key spans all 10 columns in row 2 */
  .num-enter { grid-column: span 10; }
...
<div id="mobile-panel">
  ...
  <div id="enter-row">
    <button class="num-btn num-enter yellow-btn" style="width:25%;" onclick="sendRaw('\x1b')">&#9099; Esc</button>
    <button class="num-btn num-enter yellow-btn" style="width:25%;" onclick="sendRaw('\x03')">&#8984; ^C</button>
    <button class="num-btn num-enter yellow-btn" style="width:25%;" onclick="triggerSpacebar()">Space</button>
    <button class="num-btn num-enter yellow-btn" style="width:25%;" onclick="sendRaw('\r')">&#9166; Enter</button>
  </div>
</div>
...
<script>
  ...
  // On mobile, account for visual viewport changes (virtual keyboard)
  if (isMobile && window.visualViewport) {
    window.visualViewport.addEventListener('resize', function() {
      ...
      const avail = window.visualViewport.height - headerH - panelH;
      container.style.height = Math.max(avail, 80) + 'px';
      debouncedFit();
    });
  }
  ...
  function updateActionButtons(prompt) {
    ...
    // Render normal command buttons (NO yellow) in first row
    const normalRow = '<div class="action-row">' +
      cmds.map(c =>
        '<button class="act-btn" onclick="insertCmd(\'' + c + '\')">' + c + '</button>'
      ).join('') +
      '</div>';
    // Render yellow buttons in a separate row
    const specialCmds = ['/', '?', 'neutral', 'enemy', 'friendly', 'ships', 'bases', 'planets'];
    const yellowRow = '<div class="action-row yellow-row">' +
      specialCmds.map(c =>
        '<button class="act-btn yellow-btn" onclick="insertCmd(\'' + c + '\')">' + c + '</button>'
      ).join('') +
      '</div>';
    container.innerHTML = normalRow + yellowRow;
  }
  ...
  // Initialize buttons with default prompt
  ...
  // --- Custom input buffer logic ---
  let inputBuffer = "";

  // Helper: redraw the current input line (prompt + buffer)
  function redrawInputLine() {
    // Move to new line, print prompt, print buffer
    // (Assume prompt is always at start of line)
    term.write('\r'); // CR to start of line
    // Find the prompt (PG> or Command:)
    let prompt = (currentPrompt === 'Command:') ? 'Command: ' : 'PG> ';
    // Clear line (ESC[2K)
    term.write('\x1b[2K');
  ...
  term.onData(function(data) {
    for (let i = 0; i < data.length; ++i) {
      ...
      } else if (ch === '\x1b') { // ESC — handled by attachCustomKeyEventHandler; skip duplicate
        // (ESC is sent directly in attachCustomKeyEventHandler; do nothing here)
      } else if (ch === '\x03') { // Ctrl-C — send immediately, clear buffer
        inputBuffer = "";
  ...
  // insertCmd: insert a command name into the input buffer and terminal
  function insertCmd(name) {
    inputBuffer += name;
    term.write(name);
  }

  // triggerSpacebar: insert a space into the input buffer and terminal
  function triggerSpacebar() {
    inputBuffer += ' ';
    term.write(' ');
  }

  // sendRaw: send raw character(s) directly (used for numpad Enter)
  function sendRaw(ch) {
    if (ch === '\r') {
      // Simulate pressing Enter (send buffer, clear, redraw prompt)
      if (inputBuffer.length > 0) {
        if (ws.readyState === WebSocket.OPEN) { ws.send(inputBuffer + '\r'); }
      } else {
        if (ws.readyState === WebSocket.OPEN) { ws.send('\r'); }
    ...
    } else if (ch === '\x1b') { // ESC — send directly, clear buffer
      inputBuffer = "";
      if (ws.readyState === WebSocket.OPEN) { ws.send('\x1b'); }
    } else if (ch === '\x03') { // Ctrl-C — send directly, clear buffer
      inputBuffer = "";
      if (ws.readyState === WebSocket.OPEN) { ws.send('\x03'); }
    } else if (ch >= ' ' && ch <= '~') {
      inputBuffer += ch;
