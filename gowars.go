...
func (i *Interpreter) isGameStateChangingCommand(command string) bool {
	...
	if cmdName == "admin" && len(fields) >= 2 {
		return gameStateAdminParams[fields[1]]
	}
...
func (i *Interpreter) robotsUnlocked(galaxy uint16, callerSide string) {
	...
	for _, obj := range objectsCopy {
		if obj.Galaxy == galaxy && obj.Type == "Ship" && obj.Side == "romulan" {
			// Generate random movement between -6 and 6 sectors
			deltaX := RandomInt(13) - 6
			deltaY := RandomInt(13) - 6
			targetX := obj.LocationX + deltaX
			targetY := obj.LocationY + deltaY
	...
	// --- ROMULAN TAUNT ---
	if len(romulanMoves) > 0 && RandomInt(100) < 30 {
...
func (i *Interpreter) initializeObjectsWithParams(galaxy uint16, params GalaxyParams) {
	...
	i.trackerMutex.Lock()
	now := time.Now()
	i.galaxyTracker[galaxy] = GalaxyTracker{GalaxyStart: now, LastActive: now, MaxSizeX: params.MaxSizeX, MaxSizeY: params.MaxSizeY}
	i.trackerMutex.Unlock()
}
